/* 
4- Crea una clase llamada Cuenta y va contener lo siguiente:
...
export class Cuenta {
 ...
 // 1
    constructor(nombre: string, cantidad:number, tipoCuenta: string, numCuenta: string){
        this.nombre = nombre
    ...
    depositar(deposito:number){
        if(deposito < 5){
            ...
            console.log('"El depósito se ha hecho correctamente"')
        }
    }
    ...
    retirar(valor: number){
        if(valor < 5){
            ...
            console.log("No cuenta con fondos suficientes para realizar el depósito")
        }
        else {
            //Cantidad - retiro
            this.cantidad -= valor
            console.log("Se ha hecho un retiro de $"  + valor + ", por tanto, su saldo actual es de: $" + this.cantidad)
